...
export default function Upcoming() {
  const { upcomingMovies, loading } = useContext(UserContext);
  ...
  useEffect(() => {
    if (upcomingMovies) {
      setSortedMovies([...upcomingMovies]);
  ...
  useEffect(() => {
    ...
    const fetchGenres = async () => {
      try {
        ...
        if (!abortController.signal.aborted) console.warn('Error fetching genres:', error);
      }
    };
    const fetchKeywords = async () => {
      try {
        ...
        if (!abortController.signal.aborted) console.warn('Error fetching keywords:', error);
      }
    };
    fetchGenres();
    fetchKeywords();
            ...
            {sortedMovies.map((movie) => (
              <div key={movie.id} className="col-md-3">
                <div className="card movie-card mb-3">
                  <Link
                    to={`/movie/${movie.id}/${movie.title.replace(
                      /\s+/g,
                      "-"
                    )}`}
                    className="link"
